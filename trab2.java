public class trab2 {
    ...
    static void cadastrarAlunos() {
        ...
        for (int i = 0; i < totalAlunos; i++) {
            String nome;
            do {
                nome = IO.readString("Digite o nome do aluno " + (i + 1) + ": ").trim();
                if (nome.isEmpty()) {
                    IO.println("Nome inválido. Digite um nome não vazio.");
                    continue;
                }
                if (nomeDuplicado(nome, i)) {
    ...
    static void buscarAlunoMenu() {
        ...
        String nomeProcurado = IO.readString("Digite o nome do aluno que deseja buscar: ").trim();
    ...
    static boolean nomeDuplicado(String nome, int limite) {
    ...
    static int pesquisarAluno(String nomeProcurado, String[] vetorNomes, int limite) {
        ...
        while (inferior <= superior) {
            ...
            int comparacao = vetorNomes[meio].compareToIgnoreCase(nomeProcurado);
            if (comparacao == 0) {
                return meio;
            }
            if (comparacao < 0) {
